(function(b) {
    function j(e, a) {
        if (0 === e.indexOf("#")){ 
			b('#langMenu').clone(!0).appendTo(c.empty()).show();						
			b(e).clone(!0).appendTo(c).show();
			b('#socialsMenu').clone(!0).appendTo(c).show();
		}
        else {
            if (a) {
                var d = b("<iframe />").attr({
    ...
    b.fn.pageslide = function(e) {
        this.click(function(a) {
            ...
            a.preventDefault();
            a.stopPropagation();
            /*c.is(":visible") && d[0] == m ? '' : cw.css({'min-width': cw.outerWidth()+'px'});*/
	...
	b('#search').bind("click", function(){
		if(b(this).hasClass('opened')){
		}
		else{
			b.pageslide.close();
		}
